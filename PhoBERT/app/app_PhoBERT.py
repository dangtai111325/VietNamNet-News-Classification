"""
Ứng dụng phân loại tin tức Vietnamnet — PhoBERT
...
def preprocess(title: str, content: str) -> str:
    ...
    # Title + content (khớp với pipeline training hiện tại)
...
def predict(title: str, content: str, tokenizer, model, cfg: dict,
    ...
    pred_score = float(adj_probs[pred_idx])
    score_sum = float(adj_probs.sum()) if np.isfinite(adj_probs).all() else 0.0
    decision_conf = (pred_score / score_sum) if score_sum > 0 else pred_prob

    return {
        "pred_class":      classes[pred_idx],
        "confidence":      pred_prob,                         # xác suất softmax đã temperature-scaled
        "decision_score":  pred_score,                        # điểm dùng để ra quyết định cuối
        "decision_conf":   float(decision_conf),              # tỉ lệ tương đối sau threshold
...
def show_result(res: dict, source_label: str = ""):
    ...
    decision_pct = res["decision_conf"] * 100
